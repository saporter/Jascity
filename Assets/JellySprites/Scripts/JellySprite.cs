...
#region PUBLIC_CLASSES
    ...
    /// <summary>
    /// Convenience function added (by Scott) to toggle rotation on/off
    /// </summary>
    /// <param name="toggle"></param>
    public void LockRotation(bool toggle)
    {
        m_LockRotation = toggle;
        UpdateRotationLock();
    }
		...
		foreach(JellySprite.ReferencePoint referencePoint in ReferencePoints)
		{
			if(!referencePoint.IsDummy)
			{
				...
				{		
					...
					if(Physics2D.OverlapCircle(bodyPosition /*+ new Vector2(0, -circleCollider.radius * 0.1f)*/, circleCollider.radius*1.1f, groundLayer))
					{
						numGroundedBodies++;
